...
describe('Counter Store', () => {
    ...
    const mockActivePost = {id: 'test', title: 'test2', body: 'test2', reactions: 5};
    const mockApiCallWithPosts = (emptyPosts = false) => vi.fn(async () => {
        return Promise.resolve(
            { posts: emptyPosts ? [] : mockPosts, total: emptyPosts ? 0 : 1}
        );
    });
    ...
    test('setActivePost',  () => {
        ...
        it('should set active post with sending request', async () => {
            const store = usePostsStore(mockApiCallWithPosts(), mockApiCallWithPost)();
            await store.setAvailablePosts(0);
            await store.setActivePost(mockPosts[0].id);
            expect(store.activePost).toStrictEqual(mockPosts[0]);
